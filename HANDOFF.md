...
**Last updated:** 2026-07-02 (session 20 — the front decides the ground + size-scaling government outlays) · **Tests:** 1478 passing · **Version:** v1.6.0 · **Status:** Phases 1–18 complete; deep-expansion underway; **first human playtest completed — "trending right direction."** **LATEST SESSION (session 20 — THE FRONT DECIDES THE GROUND + THE MATURE STATE PAYS ITS WAY; 2 feature commits):** (1) FRONT→OCCUPATION: the occupation gate in `tickPlayerWar` now reads the integrated `front.position` (≥ `FRONT_OCCUPY_THRESHOLD` 35) instead of the noisy monthly `w.score`, so a march is taken on a SUSTAINED advance — never a lucky roll while the line is contested — and a momentary score dip no longer cedes ground mid-advance. The front's posture moves the odds: breakthrough takes at `MARCH_TAKE_CHANCE_BREAKTHROUGH` 0.5 (vs 0.3); collapse cedes at `MARCH_CEDE_CHANCE_COLLAPSE` 0.45 (vs 0.25). Draw-then-decide: BOTH rolls drawn every war month so the RNG draw count never depends on front state (the session-17 draw-order trap, dodged by construction). War-panel "columns probe their marches" hint reads the front + threshold. Byte-identical (playerWar-gated; **181y×2 A/B headless diff vs HEAD matched byte-for-byte**). `tests/front-occupation.test.ts` +9. (2) SIZE-SCALING GOVERNMENT OUTLAYS (economy-realism inc 3 — deferred since session 12, **unblocked by the playtest**): an income-tax state now spends `max(0, treasury − GOV_OUTLAY_RESERVE_MONTHS(6)×GDP) × GOV_OUTLAY_SKIM(0.2) × min(1, towns/GOV_OUTLAY_TOWNS_FULL(8))` each month — the ministries/pensions/procurement a large developed nation actually runs, so the 3%-of-GDP income-tax levy no longer piles into an unbounded hoard. SPIRAL-SAFE by construction (the session-12 deficit-death-spiral warning): only the surplus above the generous 6-month reserve is touched, a strained budget pays nothing, and it composes with the autoplay statehood sink (that sink's tighter 1.5-mo reserve drains first). Transient `lastGovOutlay` (NOT serialized — no schema change) + a Finance→Treasury "state apparatus −N/mo" line with tooltip. Gated on the `income_tax` civic the default sweep player never researches → byte-identical (same A/B diff). `tests/gov-outlay.test.ts` +11 (exact-formula post-hoc reconstruction, reserve-floor safety, ramp, convergence, not-serialized). Gates: tsc clean; **1478 tests**; serialize-determinism byte-identical; bench PASS (worst 14.4ms solo); vite build green; headless 181y×2 **BYTE-IDENTICAL to HEAD**. ⚠️ Gotcha for the next session: `npm run sim -- <N> <runs>` — N is **YEARS not days** (the CLAUDE.md quick-ref says days; passing 3650 runs a 3650-YEAR sweep that pegs a core for half an hour). **PRIOR SESSION (session 19 — AGENDA-DRIVEN PEACE; 2 feature commits):** (1) AgendaKind enum + AGENDA_PEACE_RESISTANCE: new structured `AgendaKind` type and `ARCHETYPE_AGENDA` mapping let each rival archetype charge quantitatively more or less war-score per peace term. Hegemon resists giving up border provinces (+15); trading republic resists reparations (+15); crusader state resists regime change (+20); opportunist discounts both (−5). `peaceBasketAsk` adds the resistance per term; `peaceCounter` now sorts by effective (agenda-adjusted) score so rivals concede their least-valued terms first. UI peace-term buttons show per-archetype resistance notes. Pure `rivalAgendaKind()` derived from archetype — no new serialized state. Byte-identical (all hooks in player-query methods only). `tests/agenda-peace.test.ts` +23. (2) AGENDA_TABLE_COST: `{ isolation: +10, opportunism: −5 }` wired into `tableCost()` — hermit kingdoms cost more diplomatic capital to approach, opportunists are eager dealmakers. UI agenda tooltip surfaces "Difficult to approach" / "Eager dealmakers" note. `tests/agenda-peace.test.ts` +4 (27 total). Both commits byte-identical in headless. **PRIOR SESSION (session 18 — THE WAR CYCLE CLOSES; 2 feature commits):** (1) Rival revanchism: rivals who lost a war to the player now remember it and can re-declare using the 'revanchism' CB (highest war-support, 85) after `REVANCHISM_BUILDUP_YEARS=5` with hostile relations (<−40) and no pact; probability weighted by grudge+risk weights × archetype multiplier. Distinctive log: "marches for revenge — they have not forgiven their defeat." Byte-identical: `warScars` always empty in headless. `tests/revanchism.test.ts` +14. (2) Front peak → peace leverage: `peaceBasketAsk` now subtracts `floor(max(0, front.peak) × FRONT_PEAK_LEVERAGE_SCALE=0.15)` alongside the occupation discount. A breakthrough peak (80) shaves ~12 off the basket. Negative peaks yield zero leverage. UI tooltip shows full discount breakdown. `tests/peace-leverage.test.ts` +8. Both byte-identical in headless. **PRIOR SESSION (session 17 — THE WAR BECOMES LEGIBLE; 2 feature commits):** (1) `PlayerWar.front` was a write-only stub mirroring the noisy war score; now it's a lagging integrator (`advanceFront`, eases `FRONT_LAG`=0.35/mo toward the score) with a high-water `peak` and a derived `FrontPhase` (breakthrough/advancing/contested/falling_back/collapse) — the war reads as a moving line, not the last roll; it narrates posture changes to the event log and gets its own war-panel meter. (2) `warScars` recorded every war but was never shown; now `WarScar` carries the front's `frontPeak` and the diplomacy panel shows a per-rival `⚔ war record: 2W 1L · revanchism available` line. Both draw NO RNG and touch no RNG-gated branch → every autoplay/headless sweep (which never holds a `playerWar`) stays byte-identical; serialize-determinism byte-identical, bench worst 16.3ms, vite build green. New `tests/front-line.test.ts` (11). Full session-17 write-up in the `## Recent session (2026-07-02)` section below; session-16 (the Gilded Century) detail in `.handoff.md` (top entry). **PRIOR SESSION (session 16 — THE GILDED CENTURY; 5 commits):** full write-up in `.handoff.md` (top entry) — summary: a live `:root` design-token system (Civ VI direction: navy surfaces, gold gilding, parchment text) replaced the scattered light-indigo theme; the dead `.cv-app` token block was deleted (live classes ported); every panel family re-tokened; the wrapped-sentences problem killed at the root (fixed widths + break-word → `width:max-content` with caps + styled scroll, 56ch prose measure); unified controls (gilded primaries, brass sliders, `.stepper` +/− keys, `.meter` bars, engraved `.panel-title` nameplates); World Dynamism new-game toggles expose consumerDemand + rivalClimateResponse as SERIALIZED campaign options (persistence test intentionally flipped; headless env flags still override — default sweep byte-identical); new-mechanics UI: ARSENAL readout (war room + Economy→Supply), intel-gated rival ⚒ arms line, Economy→Wonders tab with 🚀 THE SPACE RACE; scenario list got real cards (its classes had never been styled); NEW scripts/shoot-ui.mjs Playwright harness = the repeatable visual gate (10 screenshots verified). Gates: tsc, 1393 tests, default sweep byte-identical to session-15 baseline, bench 6.9ms. Known UI follow-ons + gotchas (fake E/S/R hotkeys, RegionalFaction vs RivalNation id-space in drawRivalPanel, panel stacking) recorded in the baton.
...
**Activated the one genuinely-inert D2-mil scaffold.** `PlayerWar.front` was a write-only stub — `w.front = { position: w.score }` mirrored the noisy monthly war score and nothing read it (the d2-mil scaffold test even skipped it: `foundColony` spawns 0 rivals, so its inject-war path never fired). Now the front is a **lagging integrator** of the war score, so the line reads as a coherent moving front rather than the last roll.

**Increment 1 — the front line (commit 1):**
...
- `systems/military.ts` (`tickPlayerWar`): the front write now calls `advanceFront(w.front, w.score)` and **narrates a posture change** to the event log (only on transition, `{0}`→rival name). No new RNG draw — the integrator is pure arithmetic on the already-updated `w.score`.
- `ui/regionview.ts`: the war panel gains a **`front` meter row** (position → 0..100 bar, tone by phase, label = posture, tooltip shows the deepest advance) beside the raw war-score row.
- Stale-comment cleanup: removed the "all 1.0 now (no-op scaffold)" doc above `WAR_SUPPORT_DECAY_MULT` (it was tuned live long ago: 0.55 fascist … 1.5 direct_democracy, already plugged into `tickWarSupport`).

**Increment 2 — the war record (commit 2).** `warScars` records every war (rival, outcome, casualties, duration) and drives revanchism (region.ts:10596/10893) + a regime event (regime.ts:85), but the player **never saw it** — no UI anywhere. Now: `WarScar` gains an optional `frontPeak` (the front's high-water mark, set in `recordWarScar` + the military.ts status_quo push), and the **diplomacy panel** shows a per-rival `⚔ war record: 2W 1L · revanchism available` line (regionview.ts, in the RivalNation block beside the war verb — `rv` is a real RivalNation so `rv.id` matches `warScars.rivalId`, sidestepping the RegionalFaction id-space trap). Fully **additive**: `frontPeak` is optional (old-save backfill), the UI reads the already-public `warScars` (no getter), and nothing in the tick path changed → byte-identical in every sweep. `+1 test` (status_quo end path records a numeric `frontPeak`).

**Why it's safe / determinism.** Both increments run only inside `tickPlayerWar` / at war-end / in read-only UI, gated on a `playerWar` that **no autoplay/headless sweep ever holds** → every balance/long-run/determinism run is byte-identical. The change draws no RNG and touches no RNG-gated branch, so draw ORDER is untouched (the trap that would have broken determinism — gating an existing `rng.chance()` on a new condition — was deliberately avoided; the score/occupation/peace logic is untouched). `playerWar` serializes wholesale → `front` persists automatically; pre-front saves self-heal on the next war tick (`advanceFront(undefined, score)` starts the line on the score).
