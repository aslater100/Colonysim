...
- ◑ **Leg 1 — world market: SUBSTANTIALLY COMPLETE.** ✅ `worldGoodPrice`/`worldGoodSupply`/`worldGoodDemand`/
  ...
  **Leg-1 NEXT:** a forced-specialisation/downturn LIVE probe that drives `localGoodsScarcity>0` so the
  great-power tilt's arbitrage reroute is MEASURED end-to-end (the long-deferred "dormant→measured" debt);
  optionally feed great-power posture into the MACRO coupling (cost-push/output-drag) — a deliberate re-baseline.
