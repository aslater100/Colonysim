...
- **Remaining spatial-AI follow-on:** (a) tune lean magnitudes (`BUILD_LEAN_*`,
  ...
  exercise isn't single-town — a bigger re-baseline. Building/district choice + expansion
  siting (rivals) and town development (rivals + flag-gated player) are all shipped (✅ above).
...
- ◑ **Leg 1 — world market: SUBSTANTIALLY COMPLETE.** ✅ `worldGoodPrice`/`worldGoodSupply`/`worldGoodDemand`/
  ...
  **Leg-1 NEXT (the handed-off increment 2 — the bigger re-baseline that makes "one global economy" FELT):** make
  the PER-TOWN `localGoodPrice` flow-based too (today its LOCAL part is stock-based → swamped by unbounded
  inventory) + a consumption SINK that drains per-town stock, so an under-producing/specialised town actually runs
  short → world scarcity → local price → arbitrage → REAL on-map flows → the economy RESPONDS to global scarcity.
  Then consider feeding world tightness / `localGoodsScarcity` into the MACRO coupling (cost-push/output-drag) and
  flipping `consumerDemand` ON by default — both deliberate re-baselines, want a human downturn playtest.
