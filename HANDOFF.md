...
## Recent session (2026-06-30) — Track-C decomposition: 3 tick subsystems lifted out of the RegionSim monolith (PR #323)
...
**Shipped 3 extractions (region.ts −603 lines: 15,843 → 15,240), each its own commit, 1307 tests green + tsc + determinism byte-identical at every step:**
...
Ordered cleanest-first. Positions are **as of commit `1830fbd`** — re-`grep` signatures by name first (line numbers shift after each extraction; names are stable).
- **`systems/diplomacy.ts` — the biggest CLEAN win (~250 lines, NO test-file changes: all 7 fns have 0 ui/test call-sites).** Move: `tickRivalProvinceGovernance` (was @8734, standalone), `updateDiplomacy` (@11046), `tickForeignRelations` (@11185), `tickRivalEspionage` (@11243), `tickRivalTradeBlocActivity` (@11291), `tickSanctions` (@11395), `endForeignWar` (@11428). These are **interleaved** with must-stay methods → ~5 separate deletion blocks (don't move `noteHistory`@11134 / `recordWarScar`@11140 / `changeRegime`@11166 / `rivalImposeSanction`@11387 / `startForeignWar`@11401 / `availableCasusBelli`@11450). Expose the must-stay helpers the moved bodies call (`noteHistory`/`recordWarScar`/`changeRegime`/`rivalImposeSanction`) + field `nextRivalBlocId` (@3274). Verify exact expose list against the bodies via tsc.
- **`systems/monetary.ts` (medium, ~210 lines).** Move `tickMonetary`, `tickFX` (tick-only) + the private helper `computeCreditRating`. `computeExchangeRate` is read directly by `tests/phase15.test.ts` and `devalue`/`switchCurrencyRegime` are player actions → **stay**. `tests/goods-prices.test.ts` defines a local `tickMonetary` wrapper that should import the new free fn.
