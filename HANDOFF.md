...
kept) → **slice 4 — activate via specialisation** (Phase C terrain yields, or a demand mechanic) →
44-good catalog; demand-aware (global) shipping; E2 R/C/I/O demand + land-value grid maps (trips the
save-size guard by design); `drawBackdrop` parallax compositing.
