...
**★ CURRENT TOP PRIORITIES (session 11 — supersede the dated list below):**
1. **Autoplay state-budget SINK (the new top rock).** `autoplayStatehood` works and lights up measurable cost-push inflation (infl% 4.3–5.5 vs the pinned 2.0), but a flush autoplay STATE lets income-tax revenue (+3%GDP/mo) outrun its dev spend → treasury runs to ~40 months of GDP (£8–9bn). Give the autoplay state a proportional spending sink (dev/links/military/services from surplus, or a modeled gov-spending outflow) so treas/GDP stays bounded. ONLY then can `autoplayStatehood` (+`consumerDemand`) flip DEFAULT-ON in the sweep. Evidence & wiring: `advanceAutoplayStatehood` (systems/rival-ai.ts), the flag in region.ts, `SIM_AUTOPLAY_STATEHOOD=1`.
2. After (1): flip `autoplayStatehood`+`consumerDemand` default-ON (re-baseline) and tune `FINAL_SHORTFALL_OUTPUT_DRAG`/`_INFLATION`.
