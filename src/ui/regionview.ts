...
export class RegionView {
  ...
  private drawStatePanel(): void {
    ...
    const treasuryBody =
      ...
      (r.agriClimateMult() < 1
        ...
        : '') +
      (r.industryClimateMult() < 1
        ? `<p class="insp-skills" title="Sustained warming above +${INDUSTRY_BROWNOUT_THRESHOLD}°C degrades grid reliability and causes heat stress in manufacturing — industrial output falls. Cut emissions to reduce the drag.">🏭 industry output −${Math.round((1 - r.industryClimateMult()) * 100)}% brownout drag</p>`
        : '') +
  ...
  private warHtml(): string {
    ...
    const agendaResist = AGENDA_PEACE_RESISTANCE[rivalAgendaKind(rv)];
    const termBtns = (Object.keys(PEACE_TERMS) as PeaceTerm[])
      .map((t) => {
        ...
        const delta = agendaResist[t] ?? 0;
        const resistNote = delta > 0 ? ` ${rv.name} will resist (${RIVAL_ARCHETYPES[rv.archetype].name}: +${delta}).`
          : delta < 0 ? ` ${rv.name} is more willing (${RIVAL_ARCHETYPES[rv.archetype].name}: ${delta}).`
          : '';
        return `<button class="mini war-term-btn${picked ? ' toggle-on' : ''}" data-term="${t}" ${blocked ? 'disabled' : ''} ` +
          `title="${PEACE_TERMS[t].desc}${blocked ? ' (you must hold a march to claim it)' : ''}${resistNote} (effective cost ${PEACE_TERMS[t].score + delta})">` +
    ...
    const offerBtn = picks.length > 0
      ? `<button class="mini war-offer-btn" ${w.score >= (ask ?? 0) ? '' : 'disabled'} ` +
        `title="Put the basket on the table.${leverageTip}">offer terms (ask ${ask})</button>`
    ...
    return this.arsenalHtml() +
      ...
      (() => {
        ...
        return `<div class="bar-row" title="The front line — the war score with inertia, so it reads as a moving line rather than the last roll. Deepest advance reached: ${Math.round(front.peak)}${peakLeverage > 0 ? ` (−${peakLeverage} from peace ask)` : ''}.">` +
